...
export const triggerRecurringTransaction = inngest.createFunction(
  {
    ...
    type: "Trigger Recurring Transactions",
  },
  { cron: "0 0 * * *" },
  async ({ step }) => {
...
export const processRecurringTransaction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!event?.data?.transactionId || !event.data?.userId) {
      ...
      return { error: "Missign required event data" };
    }
      ...
      await db.$transaction(async (tx) => {
        ...
        await tx.transaction.update({
          ...
          data: {
            ...
            nextRecurringDate: calcNextRecurringDate(
              new Date(),
...
function calcNextRecurringDate(startDate, interval) {
